...
exports.recordDownload = exports.getDocumentChecklist = exports.getRequiredDocuments = exports.downloadFile = exports.getDownloadItem = exports.getDownloadCategories = void 0;
const downloads_1 = require("../data/downloads");
const getDownloadCategories = async (req, res) => {
    try {
        const { language, stage, required } = req.query;
        let filteredCategories = downloads_1.downloadData.categories.filter(category => category.items && category.items.some(item => item.isActive));
        if (language) {
            filteredCategories = filteredCategories.map(category => ({
                ...category,
                items: category.items.filter(item => item.language === language || item.language === 'en')
...
exports.getDownloadCategories = getDownloadCategories;
const getDownloadItem = async (req, res) => {
    try {
        const { id } = req.params;
        let foundItem;
        for (const category of downloads_1.downloadData.categories) {
...
exports.getDownloadItem = getDownloadItem;
    ...
    try {
        ...
        res.json({
            ...
            data: {
                ...
                mobilePaymentRequirements: downloads_1.downloadData.mobilePaymentRequirements,
                documentRequirements: downloads_1.downloadData.documentRequirements,
                calibrationProcess: downloads_1.downloadData.calibrationProcess,
                appDownload: downloads_1.downloadData.appDownload,
